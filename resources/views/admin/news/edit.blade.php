        ...
        <div class="col-sm-12">
            <div class="float-right page-breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="{{ route('admin.news.edit',$news->slug) }}">Edit</a></li>
                </ol>
            </div>
            <h5 class="page-title"> {{ getLanguage('news').' '.getLanguage('edit') }} : {{ $news->title }}</h5>
        </div>
    </div>
    <!-- end row -->
                ...
                <div class="card-body">
                    ...
                    <form method="post" action="{{ route('admin.news.update',$news->slug) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="title" value="{{$news->title}}" id="title" class="form-control" required>
                        </div>
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <div class="btn-group btn-group-toggle form-control" style="height: 49px"  data-toggle="buttons">
                                        ...
                                        <label class="btn btn-light @if($news->status == "Active") active @endif ">
                                            <input type="radio" name="status" id="option1" @if($news->status == "Active") checked @endif value="Active"> Active
                                        </label>
                                        <label class="btn btn-light @if($news->status == "Inactive") active @endif">
                                            <input type="radio" name="status" @if($news->status == "Inactive") checked @endif id="option2" value="Inactive"> Inactive
                                        </label>
                                    </div>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="image_preview">
                                    <div class="z-depth-1-half mb-4" style="text-align: center;">
                                        <img id="preview_image" src="@if(isset($news->image)) {{asset($news->image)}} @else https://mdbootstrap.com/img/Photos/Others/placeholder.jpg @endif" class="avatar-pic img-fluid" title="Click To Upload news Photo"
                                             alt="news Image">
                                    </div>
                            ...
                            <div class="col-md-9">
                                <div class="form-group">
                                    ...
                                    <textarea name="contents" id="content" class="summernote" >{!! $news->content !!}</textarea>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    function uploadimages(e){
        ...
        if(file){
            ...
            reader.addnewsListener('load',function(){
